...
import com.forlost.zhongtuo.helper.SecuredPreferenceHelper;
...
import com.forlost.zhongtuo.ui.base.AdBaseActivity;
import com.forlost.zhongtuo.R;
import com.google.gson.JsonObject;
import java.util.Date;
import static com.forlost.zhongtuo.conf.Config.DIALOGID_KEY;
import static com.forlost.zhongtuo.conf.Config.DIALOGNAME_KEY;
import static com.forlost.zhongtuo.conf.Config.DIALOGTYPE_KEY;
import static com.forlost.zhongtuo.conf.Config.MESSAGE_TYPE_KEY;
...
//        MyMessageInput myMessageInput = findViewById(R.id.messageInput);
        ...
        messageInput.setOnSubmitListener(new MessageInput.OnSubmitListener() {
            @Override
            public boolean onSubmit(final Message message) {
                //消息状态
                message.status = 0;
                //存数据库
                //https 请求服务器
                JsonObject jsonObject=new JsonObject();
                //将message对象转成jsonObject
                DtdHttpHelper.getInstance().doPost("",jsonObject,JsonObject.class,new DtdHttpCallback<JsonObject>(){
                    @Override
...
//        });
    ...
    private void sendMessage(int messageType, JsonObject jsonObject) {
        final com.forlost.zhongtuo.room.Message message = new com.forlost.zhongtuo.room.Message();
        message.dialogType = dialogType;
        message.createAt = new Date();
        message.fromAccount = SecuredPreferenceHelper.getInstance().getSharedPreferences().getInt(Config.DTD_USER_UID_KEY, 0);
    ...
    private void afterSendMessage(com.forlost.zhongtuo.room.Message message, String serverMsgId) {
