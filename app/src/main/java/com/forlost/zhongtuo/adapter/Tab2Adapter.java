 ...
 * 任务列表适配器
 */
public class Tab2Adapter extends RecyclerView.Adapter<Tab2Adapter.myViewHodler> {
    private ArrayList<Task> myTaskList;
    private Context context;

    //创建构造函数
    public Tab2Adapter(Context context, ArrayList<Task> myTaskList) {
        //将传递过来的数据，赋值给本地变量
    ...
    public myViewHodler onCreateViewHolder(ViewGroup parent, int viewType) {
        //创建自定义布局
        View itemView = View.inflate(context, R.layout.tab2, null);
        return new myViewHodler(itemView);
    }

    @Override
    public void onBindViewHolder(@NonNull Tab2Adapter.myViewHodler holder, int position) {
